...
func LoadSavedData() (map[string]*model.ChromeInstallerInfo, error) {
	f, err := os.ReadFile(dataPath)
	if err != nil {
		return nil, errors.New(fmt.Sprintf("read file error: %v", err))
	}
...
func SaveMarkdown(data map[string]*model.ChromeInstallerInfo) error {
	...
	buf.WriteString("本工程是 Bush2021/chrome_installer Go实现，感谢原作者\n\n")
